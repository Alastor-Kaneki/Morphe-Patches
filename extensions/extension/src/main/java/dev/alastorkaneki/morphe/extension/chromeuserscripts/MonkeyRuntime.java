...
final class MonkeyRuntime implements Runnable {
    ...
    static void stop(Activity activity) {
        ...
        synchronized (RUNS) { runtime = RUNS.remove(activity); }
    ...
    static List<Userscript> matches(Activity activity) { return MonkeyStore.matching(activity, url(activity)); }

    static boolean run(Activity activity, Userscript script) {
        ChromeBridge.Page page = ChromeBridge.page(activity);
        return page != null && !page.incognito && ChromeBridge.exec(page, ScriptInjector.buildPayload(script, page.url, true));
    }
    ...
    static void refresh(Activity activity) {
        MonkeyRuntime runtime;
        synchronized (RUNS) { runtime = RUNS.get(activity); }
        if (runtime != null) { runtime.generation = -1; MAIN.removeCallbacks(runtime); MAIN.post(runtime); }
    }
